...
class TestAwsS3 < Test::Unit::TestCase
  ...
  def test_create_bucket
    ...
    assert_kind_of(
      ...
      @s3_adapter.bucket_exists?(Settings.pixta.aws.s3.bucket_name)
    )
  end
  ...
  def test_valid_s3_bucket
    assert_equal(@s3_adapter.bucket_name, Settings.pixta.aws.s3.bucket_name)
  end

  private

  def bucket
    Settings.pixta.aws.s3.bucket_name
  end

  def access_key_id
    Settings.pixta.aws.s3.access_key_id
  end

  def secret_access_key
    Settings.pixta.aws.s3.secret_access_key
  end
  ...
  def endpoint
    Settings.pixta.aws.s3.endpoint
  end
end
